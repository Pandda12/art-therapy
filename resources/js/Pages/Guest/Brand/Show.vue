...
<template>
    ...
    <GuestLayout>
        <ShopLayout :page-title="brand.name">
            <div class="text-4xl uppercase my-14">
                {{ brand.name }}
            </div>
            <div v-if="!products.length">No products in brand {{ brand.name }}</div>
            <div v-else class="flex flex-wrap gap-2.5">
                <Link
                    v-for="(product, index) in products"
                    :href="product.url"
                    class="relative flex flex-col w-1/4 bg-lite-purple-1 p-5 rounded-2xl"
                >
